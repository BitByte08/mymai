...
addSection('SERVER');
addRow('sv','\uC11C\uBC84 \uC800\uC7A5');
var MAX_ATTEMPTS=3,REQUEST_TIMEOUT=15000,SYNC_UPLOAD_TIMEOUT=60000,PAGE_CONCURRENCY=3;
...
async function fetchPage(id,url,optional){for(var attempt=1;attempt<=MAX_ATTEMPTS;attempt++){try{var r=await fetchWithTimeout(url);if(!r.ok)throw new Error('HTTP '+r.status);var t=await r.text();var info=t.length>0?(t.length>1024?(t.length/1024).toFixed(1)+'KB':t.length+'B'):'\uC5C6\uC74C';okRow(id,info);console.log('[carol]',id,url,t.length);return t;}catch(e){if(attempt<MAX_ATTEMPTS){setRow(id,'\u21BB','#facc15','\uC7AC\uC2DC\uB3C4 '+attempt+'/'+(MAX_ATTEMPTS-1));await sleep(jitter(400*attempt));}else if(optional){skipRow(id,'\uC2E4\uD328');}else{failRow(id,e&&e.name==='AbortError'?'\uC2DC\uAC04 \uCD08\uACFC':'\uB124\uD2B8\uC6CC\uD06C \uC624\uB958');}}}return '';}
...
async function collectDetails(){var reqs=collectDetailRequests(rd),out=[];var _dres=await runPool(reqs,PAGE_CONCURRENCY,async function(req){for(var attempt=1;attempt<=MAX_ATTEMPTS;attempt++){try{var r=await fetchWithTimeout(req.url);if(!r.ok)throw new Error('HTTP '+r.status);var html=await r.text();console.log('[carol] detail',req.idx,req.url,html.length);return {idx:req.idx,html:html};}catch(e){if(attempt<MAX_ATTEMPTS)await sleep(jitter(400*attempt));else console.warn('[carol] detail fetch failed',req.idx,req.url,e&&e.message?e.message:String(e));}}return null;});for(var _i=0;_i<_dres.length;_i++){if(_dres[_i])out.push(_dres[_i]);}return out;}
